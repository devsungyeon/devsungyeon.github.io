...
마이크로 컴퓨터의 버스에 접속하는 경우에 버스버퍼 용 IC를 사용 하는데 이것은 데이터 버스가 3-State 라고하는 특수한 조건을 사용하고 있습니다.　이 3-스테이트의 의미는 통상 디지탈 IC는 High 또는 Low의 두가지 값으로 작동하지만 3-스테이트는 High／Low 외에 그어느쪽도 아닌 상태, 즉 하이 임피던스 상태가 하나더 존재 하기 때문에 ３- 스테이트 라고 불리고 있습니다．

하이 임피던스 상태는 접속되어 있지 않는 것과 같은 상태를 말하는 것으로 왜 이러한 기능이 필요한가 하면 마이크로컴퓨터의 데이터 버스에는 그 구성상 여러 가지의 주변기기가 동일한 라인에 접속된 상태에서 디바이스 어드레스로 특정한 장치 하나만 데이터 버스에 실제로 접속되고 마이크로컴퓨터에 데이터를 송신 할 수 있도록 하기 때문입니다. 이러한 때는 어드레스로 지정되지 않은 주변기기는 접속되어 있지 않는 상태가 가장 좋은 상태이며 그래서 하이 임피던스 상태가 필요하게 되었고 어드레스로 지정되지 않으면 데이터 버스에는 어떤 영향도 주지 않게 되는 것입니다.
또한 IC의 입력측은 큰 부하가 되지 않아서 버스라인에 큰 영향을 주지 않기 때문에 모든 주변기기를 접속한 채로해 두고 어느 주변기기든지 데이터를 수신할 수 있게 합니다.

[참고링크](https://broneri.tistory.com/328)
