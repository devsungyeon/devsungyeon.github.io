...
==> 개체관계(E-R) 모델은 ~~논리적 데이터 모델이 아닌~~ **개념적 데이터 모델**이다. 개념적으로 표현.
...
- 데이터 모델의 종류
  ...
  - 논리적 데이터 모델 ; 개념적 모델링 과정에서 얻은 개념적 구조를 컴퓨터가 이해하고 처리할 수 있는 컴퓨터 환경에 맞도록 변환하는 과정 = 레거시 데이터 모델
